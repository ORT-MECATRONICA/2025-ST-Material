...
/////mqqtt
#define MQTT_HOST IPAddress(10, 162, 24, 3) ///se debe cambiar por el ip de meca o hall del 4
...
void fun_envio_mqtt ()
{
    ...
    if (flag_vacio==0)////si hay los envio 
    {
      ...
      snprintf (mqtt_payload, 150, "%u&%ld&%.2f&%.2f&%.2f&%u&%.2f",name_device,aux2.time,aux2.T1,aux2.H1,aux2.luz,aux2.Alarma,aux2.Corr); //random(10,50)
...
void fun_saca (){
  if (indice_saca != indice_entra)
  {
    ...
    aux2.Corr = datos_struct[indice_saca].Corr;
    flag_vacio = 0; 
...
void fun_entra (void)
{
  ...
  datos_struct[indice_entra].Corr=1.5;
  indice_entra++;
